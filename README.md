...
Day 15
------
Today I learned how to initialize matrix with zeros on one line - `[[0]*m]*n` will have always the same value in all elements. Instead I had to use `matrix = [[0 for i in range(0,matrix_size)] for i in range(0,matrix_size)]`. The project euler site now checks for solutions again. I found a bug in few problems and learned few new things among the way to fix those. I learned how to merge dictionaries. Using `lambda` got to me easier than when I used it last time and I learned a little bit about commenting python code.